...
describe("core reducer", () => {
  ...
  it("drops deprecated pane zoom state from restored snapshots", () => {
    ...
    expect(
      "zoomedPaneId" in
        ((restored.workspaces[workspaceId] as unknown as Record<string, unknown>) ??
          {})
    ).toBe(false);
  ...
  it("refreshes derived metadata only when cwd changes", () => {
    ...
    expect(
      effectsWithDerivedValues.some((effect) => effect.type === "metadata.refresh")
    ).toBe(false);
  ...
  it("jumps to the latest unread notification target across workspaces", () => {
    const state = createInitialState();
    const originalWorkspaceId = state.windows[state.activeWindowId].activeWorkspaceId;

    applyAction(state, { type: "workspace.create", name: "alerts" });

    const alertsWorkspaceId = state.windows[state.activeWindowId].activeWorkspaceId;
  ...
  it("uses numbered default workspace labels in the UI while sidebar summaries follow the first tab title", () => {
    ...
    const createdWorkspaceId = state.windows[state.activeWindowId].activeWorkspaceId;
  ...
  it("closing the active workspace promotes a remaining workspace without leaving stale ids", () => {
    ...
    const vm = buildViewModel(state);
    expect(vm.workspaceRows.every((row) => row.workspaceId !== betaId)).toBe(true);
  ...
  it("clamps sidebar width updates and restores a safe default from stale snapshots", () => {
    ...
    expect(buildViewModel(restoredSnapshot).sidebarWidth).toBe(MAX_SIDEBAR_WIDTH);
  });
