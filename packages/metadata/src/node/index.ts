...
export async function resolveGitRepository(
  ...
  try {
    ...
    const [gitDir, commonGitDir, root] = stdout
      ...
      .map((line) => line.trim())
      .filter(Boolean);
    if (!gitDir || !commonGitDir || !root) return null;
    return {
...
export async function resolveGitBranch(
  ...
  try {
    ...
    branchCache.set(cwd, { value: branch, expiresAt: Date.now() + 1_500 });
    return branch;
  } catch {
    branchCache.set(cwd, { value: null, expiresAt: Date.now() + 1_500 });
    return null;
  }
}
