...
import {
  ...
  findExtractedLauncherIconSizes,
  ...
  validateLinuxLauncherIconSizes,
...
describe("linux packaged smoke wrapper", () => {
  ...
  it("parses and validates AppImage desktop-entry identity", () => {
    expect(loadExpectedLinuxDesktopEntry()).toMatchObject({
      Name: "kmux",
      Icon: "kmux",
      StartupWMClass: "kmux"
    });
  ...
  it("builds a ledger-friendly packaged smoke preflight summary", () => {
    ...
    try {
      ...
      const desktopIdentity = {
        ...
        launcherIconSizes: [16, 24, 32, 48, 64, 128, 256, 512],
      ...
      expect(summary).toContain(
        "Launcher icon sizes: 16, 24, 32, 48, 64, 128, 256, 512"
      );
  ...
  it("accepts launcher icons that AppImageLauncher can install into GNOME hicolor", () => {
    const root = mkdtempSync(path.join(tmpdir(), "kmux-appimage-icons-"));
    try {
      for (const size of [48, 128, 256, 512]) {
        ...
        writeFileSync(path.join(iconDir, "kmux.png"), "");
      }

      const iconSizes = findExtractedLauncherIconSizes(root);
      expect(iconSizes).toEqual([48, 128, 256, 512]);
      expect(() => validateLinuxLauncherIconSizes(iconSizes)).not.toThrow();
      expect(() => validateLinuxLauncherIconSizes([48, 1024])).toThrow(
