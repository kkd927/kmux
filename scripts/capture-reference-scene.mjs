...
try {
  ...
  await dispatch({ type: "workspace.create", name: "cmux cli/unix socket" });
  view = await waitFor(
    (next) => next.activeWorkspace.name === "cmux cli/unix socket"
  );
  ...
  await queueWorkspace(
    "ssh",
    "Pushed to https://github.com/manaflow-ai/cmux/pull/239",
    "~/fun/cmuxterm-hq"
  );
  await queueWorkspace(
    "cmd shift t tabs",
