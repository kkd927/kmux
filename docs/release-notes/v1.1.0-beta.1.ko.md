이번 변경에서는 원격 호스트를 kmux 워크스페이스로 그대로 사용할 수 있고,
앱 안에서 Markdown 문서를 바로 확인할 수 있으며, 에이전트 세션을 재개할
명령어를 직접 지정할 수 있습니다.
...
이제 SSH 호스트를 kmux 워크스페이스로 직접 연결할 수 있습니다. 원격
프로젝트도 로컬과 다르지 않습니다. 터미널을 나누고, 여러 에이전트 세션을
나란히 실행하고, 앱을 다시 열어 하던 작업을 이어갈 수 있습니다.
...
2. Command Palette 또는 워크스페이스 메뉴에서
   **Convert to SSH Workspace...**를 실행합니다.
3. 연결할 호스트를 고른 뒤 현재 워크스페이스를 전환할지, 새 워크스페이스를
   만들지 선택합니다.
