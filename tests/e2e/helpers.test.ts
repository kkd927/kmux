...
const sandbox: KmuxSandbox = {
  ...
  stateDir: "/tmp/kmux-e2e-profile/state",
  dataDir: "/tmp/kmux-e2e-profile/data",
  cacheDir: "/tmp/kmux-e2e-profile/cache",
...
describe("buildElectronLaunchOptions", () => {
  it("launches the source app through the app root argument", () => {
    ...
    expect(launchEnv).toMatchObject({
      ...
      KMUX_AGENT_BIN_DIR: "/tmp/kmux-e2e-profile/data/bin",
      KMUX_NATIVE_CACHE_ROOT: "/tmp/kmux-e2e-profile/cache/native",
      KMUX_RAW_OUTPUT_ROOT: "/tmp/kmux-e2e-profile/state/pty-raw",
