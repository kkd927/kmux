...
**Claude Code、Codex CLI、Gemini CLIを並行して実行し、どれひとつ見失うことはありません。**
...
実務で **Claude Code**、**Codex CLI**、**Gemini CLI** を活用し始めたなら、すでに次の問題に直面しているはずです：3つのターミナル、3つのレート制限（Rate limit）、3つのセッション履歴、そして同じリポジトリ内でエージェント同士が競合しないように安全に分離する適切な方法がないことです。
...
- ワンクリックで過去の Claude/Codex/Gemini セッションに即座に復帰
...
右側のサイドバーパネルで、Claude Code、Codex CLI、Gemini CLI の利用状況をリアルタイムに監視します。5時間のセッションウィンドウ、週次利用状況、月次支出がすべてのプロバイダーにわたって統合計算されるため、残りのセッション予算を一目で把握できます。
...
kmuxは、3つのエージェントのローカルセッションログ（Claude: `~/.claude/projects`、Codex: `~/.codex/sessions`、Gemini: `~/.gemini/tmp`）を自動的にインデックス登録し、フィルター可能な単一のパネルに統合して表示します。

特定の行をクリックするだけで、該当セッションを即座に復元します。kmuxは、同一の `cwd`（作業ディレクトリ）の既存の画面が開いていればその画面にフォーカスし、開いていなければ新しいワークスペースを起動して `claude --resume`、`codex resume`、または `gemini --resume` を実行します。
...
2. ターミナルウィンドウで、実行したいエージェント（`claude`、`codex`、または `gemini`）を実行します。
