...
function buildZshIntegrationScript(): string {
  return [
    ...
    '    [[ "$path_entry" == "$agent_bin" ]] || next_path+=("$path_entry")',
    "  done",
    '  path=("${next_path[@]}")',
    "  export PATH",
    "  rehash",
...
function buildBashIntegrationScript(): string {
  return [
    ...
    '  [[ -n "$agent_bin" && -d "$agent_bin" ]] || return 0',
    "  local -a next_path",
    "  local path_value path_entry path_entry_is_last",
    '  path_value="${PATH-}"',
    '  next_path=("$agent_bin")',
    "  while :; do",
    "    path_entry_is_last=0",
    '    if [[ "$path_value" == *:* ]]; then',
    '      path_entry="${path_value%%:*}"',
    '      path_value="${path_value#*:}"',
    "    else",
    '      path_entry="$path_value"',
    "      path_entry_is_last=1",
    "    fi",
    '    [[ "$path_entry" == "$agent_bin" ]] || next_path+=("$path_entry")',
    '    [[ "$path_entry_is_last" == "1" ]] && break',
    "  done",
    '  PATH="$(IFS=:; printf "%s" "${next_path[*]}")"',
    "  export PATH",
    "  hash -r",
    "}",
    "",
...
function buildFishIntegrationScript(): string {
  return [
    ...
    '  set -l _kmux_next_path "$_kmux_agent_wrapper_bin"',
    "  for _kmux_path_entry in $PATH",
    '    test "$_kmux_path_entry" = "$_kmux_agent_wrapper_bin"; or set -a _kmux_next_path "$_kmux_path_entry"',
    "  end",
    "  set -gx PATH $_kmux_next_path",
    "  set -e _kmux_next_path",
    "  set -e _kmux_path_entry",
    "end",
...
function buildCodexWrapperScript(): string {
  return [
    ...
    "    esac",
    "  done",
    '  _kmux_codex_major="${_kmux_codex_version%%.*}"',
    '  _kmux_codex_rest="${_kmux_codex_version#*.}"',
    ...
    '  KMUX_CODEX_HOOKS_FEATURE="$(kmux_resolve_codex_hooks_feature)"',
